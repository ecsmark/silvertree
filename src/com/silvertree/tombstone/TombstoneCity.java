...
public class TombstoneCity {
    ...
    public TombstoneCity(CVirtualTi* pTI99)
    {
        ...
        Schooners = 0 ;
        Ship = SHIPRT ;   // current ship character
    ...
    public boolean PlayDay(int day)
    {
        ...
        final BYTE     sprite[] ={0xf7, 0xd5, 0xc5, 0xf1, 0xf7,0xf7,0x81,0	};
        ...
        gameBoard.Video()->Char(160, sprite) ;
        CreateSprite() ;           // establish sprite attributes

        keydep() ;
        m_nGencur = PLAYAREABEG ;
        TITimer = GetTickCount() ;
        int timflg = 0 ;
        KEYCODE key = 0 ;      /* Last key pressed     */
        ...
        {
            key =scan() ;
            if (key == TIKEYREDO || key == TIKEYBACK)
            {
                bEndGame = true ;
                bDoneWithDay = true ;
            }
            else if (LMonct == 0 && key == FIREKEY && SMonct < 10)
            {
                timflg = 0 ;
                ...
                {
                    bDoneWithDay = true ;
                    break;       // if no monsters and no generators then begin new day
                }
            }
            else
            {
                //  4 secs has elapsed 
                if (GetTickCount() - TITimer >= N4SECS)
                {
                    TITimer = GetTickCount()  ;
                    if (timflg == TIMLEV)
                    {
                        ...
                        // ------------------------------------------------
                        //   find generator and release monster 
                        // ------------------------------------------------ 
                        if (generateLargeMonsters() && SMonct == 0)
                        {
                            genSmallMonsters() ;
                        }
                        continue ;
                    ...
                    ++timflg ;
                    if (timflg == TIMLEV && LMonct < MAXMNL)
                    {
                        // ------------------------------------------------ 
                        // find generator and highlight but dont release  
                        // monster yet.
                        // ------------------------------------------------ 
                        gameBoard.safeAreaBlueOnLightBlue() ;
                        Sprflg = 1;
                ...
                if (m_nSpeed == 1)
                {
                    killtime(5000) ;    // ship  moves twice as fast as MORG level 1 
                    scan() ;
                }
...
//  -----------------------------------------------------------------------  
    ...
    boolean generateLargeMonsters()
    {
        ...
        while (++m_nGencur != Genlas && !(generatorFound && releasePointFound))
        {
            ...
            unsigned int  c  = gameBoard.GetChar(m_nGencur) ;
            if (c == GRAVE)
            {
                /* ------------------------------------------------------------ */
...
// --------------------------------------------------------------------------------
    ...
    void checkForAdjacentGraves( int grave, bool retry)
    {
...
// --------------------------------------------------------------------------------
    ...
    void dispLevelMenu()
    {
        static char szRule1[] ={"MOVE SCHOONER\x82 ARROW KEYS"} ;
        static char szRule2[] ={"FIRE MISSILE \x82Q/Y/INSERT"} ;
        static char szRule3[] ={"] SAGUARO    \x82  0 POINTS"};
        static char szRule4[] ={"\x090 TUMBLEWEED \x82 100 POINTS"};
        static char szRule5[] ={"p MORG       \x82 150 POINTS"};
        static char szRule6[] ={"RESTART GAME \x82REDO"};
        static char szRule7[] ={"SELECT LEVEL \x82 BACK"};
        static char szRule8[] ={"PANIC BUTTON \x82SPACE BAR"};
        static char szOut11[] ={"PRESS ANY KEY TO CONTINUE"};
